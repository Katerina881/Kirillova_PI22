...
namespace ZavodConservView
{
    public partial class FormClientOrders : Form
    {
        ...
        private void reportViewer_Load(object sender, EventArgs e)
        {
            try
            {
                var dataSource = logic.GetConservComponent();
                ReportDataSource source = new ReportDataSource("DataSetConservComponent", dataSource);
        ...
        private void ButtonToPdf_Click(object sender, EventArgs e)
        {
            using (var dialog = new SaveFileDialog { Filter = "pdf|*.pdf" })
            {
                if (dialog.ShowDialog() == DialogResult.OK)
                {
                    try
                    {
                        logic.SaveConservComponentToPdfFile(new ReportBindingModel
                        {
